/* Elite Edge Advisors — ported from the retired elite-edge-advisors site.
   Kept: the bet-display home screen (AccountScreen/TodaysBets.js, verbatim) and
   the MyBookie JSON input (./InputBets.jsx). Everything else was left behind.
   Port changes only: MUI v9 Grid `size` API, react-material-ui-carousel replaced
   by the tiny AutoCarousel below, DataContext/isDev removed (prod URLs only). */
...
import InputBets from "./InputBets.jsx";

// react-loading crashes under React 19 (renders as an object — React error #130),
// so the loading spinner is a bare CSS one instead.
function Spinner() {
  return (
    <>
      <style>{`@keyframes eea-spin { to { transform: rotate(360deg); } }`}</style>
      <div
        style={{
          width: 150,
          height: 150,
          ...
          borderRadius: "50%",
          animation: "eea-spin 0.8s linear infinite",
        }}
      />
    </>
  );
...
// Minimal stand-in for react-material-ui-carousel (which needs MUI v5):
// cycles through its children on a fixed interval, no controls.
function AutoCarousel({ children, interval = 6000 }) {
...
// A game is "done" per ESPN's canonical signals (state "post" / completed),
// which covers every sport's final variants — including soccer's
// STATUS_FULL_TIME / STATUS_FINAL_AET / STATUS_FINAL_PEN, which are NOT "STATUS_FINAL".
const isGameDone = (status) =>
  status?.type?.state === "post" ||
  status?.type?.completed === true ||
  ["STATUS_FINAL", "STATUS_FULL_TIME", "STATUS_FINAL_AET", "STATUS_FINAL_PEN"].includes(status?.type?.name);
...
function TodaysBets() {
  ...
  const getInningBackgroundColor = (won, bet, index, type) => {
    ...
    } else if (isGameDone(bet.status) && won) {
  ...
  const getTodaysBets = async () => {
    ...
    try {
      ...
      const scoresSoccer = await getFinalScoresSoccer(moment().format("YYYYMMDD"));
      const scoresWNBA = await getFinalScoresWNBA(moment().format("YYYYMMDD"));
      const scores = [
        ...scoresMLB,
        ...scoresNHL,
        ...scoresNFL,
        ...
        .then((data) => {
          ...
          allTickets.forEach((ticket) => {
            ...
            const finals = betsWithScores.filter(
              (bet) =>
                isGameDone(bet.score?.status) ||
                bet.score?.status.type.name === "STATUS_POSTPONED" ||
                bet.score?.status.type.name === "STATUS_DELAYED",
            );
            // Anything with a score that isn't scheduled or done is treated as in progress.
            // (Avoids dropping legs for sport-specific live statuses like STATUS_FIRST_HALF /
            // STATUS_SECOND_HALF in soccer, which weren't in the old allowlist.)
  ...
  const getFinalScoresSoccer = async (date) => {
    const events = await fetch(
      `https://site.api.espn.com/apis/site/v2/sports/soccer/fifa.world/scoreboard?dates=${moment(date).format("YYYYMMDD")}`,
    )
      .then((response) => response.json())
      .then((data) => data.events ?? [])
      .catch(() => []);

    // ESPN's soccer scoreboard omits per-half linescores (competitors only carry a
    // total score). The per-event summary endpoint does include them, so fetch each
    // game's summary and merge the linescores onto the matching competitors by id.
    await Promise.all(
      events.map(async (event) => {
        try {
          ...
          const summaryCompetitors = summary?.header?.competitions?.[0]?.competitors ?? [];
          const linescoresById = {};
          summaryCompetitors.forEach((c) => {
            if (c?.id && c?.linescores) linescoresById[c.id] = c.linescores;
          });
          event.competitions?.[0]?.competitors?.forEach((c) => {
            if (linescoresById[c.id]) c.linescores = linescoresById[c.id];
          });
        } catch {
          // Leave the event without linescores on failure — the total score still renders.
        }
      }),
    );
  ...
  const getFinalScoresWNBA = async (date) => {
    return await fetch(
      `https://site.api.espn.com/apis/site/v2/sports/basketball/wnba/scoreboard?dates=${moment(date).format("YYYYMMDD")}`,
    )
      .then((response) => response.json())
      .then((data) => data.events ?? [])
      .catch(() => []);
  ...
  const getLinescore = (linescore, bet) => {
    // Soccer linescores use `displayValue` (others use `value`), and ESPN inserts a
    // "-1" sentinel for non-scored slots (e.g. the extra-time gap in penalty games).
    const val = (x) => {
      const v = x.value ?? x.displayValue;
      return v === "-1" || v === -1 ? "-" : v;
    };
    if (bet.score?.competitions[0].format.regulation.periods === 9) {
      if (linescore?.length > 8) return linescore.map(val);
      else if (linescore?.length < 9) {
        const bets = new Array(9 - linescore?.length).fill("");
        bets.unshift(...linescore.map(val));
        return bets;
      } else {
      ...
      if (linescore?.length > 2) return linescore.map(val);
      ...
      if (linescore?.length === 4) return linescore.map(val);
  ...
  const getGameStatus = (score) => {
    ...
    const isSoccer = score.links?.[0]?.href?.includes("soccer");
    ...
    return (
      ...
      : periods === 2 && isSoccer ?
        // Live soccer: show the running game clock (e.g. "63'") instead of the
        // period label; "HT" at the break; the final label (FT/AET/FT-Pens) once done.
        st.type.state === "post" ? st.type.shortDetail
        : st.type.name === "STATUS_HALFTIME" ? "HT"
        : st.displayClock || st.type.shortDetail
  ...
  // A ticket is "live" if at least one of its games is currently in progress,
  // i.e. has a score that isn't scheduled or final (matches the grouping in getTodaysBets).
  const ticketHasLiveGame = (ticket) =>
    ticket?.some((bet) => {
      const name = bet.score?.status?.type?.name;
      return (
        name &&
  ...
  // Tickets to actually render. When the toggle is on, drop any ticket with no live game.
  const visibleBets = hideNotInProgress ? todaysBets.filter(ticketHasLiveGame) : todaysBets;
  ...
  if (isLoading) {
    ...
    return (
      <div style={{ backgroundColor: "black" }}>
        ...
        <button
          onClick={() => setHideNotInProgress((prev) => !prev)}
          style={{
            position: "fixed",
            bottom: "calc(100px + env(safe-area-inset-bottom))",
            right: 16,
            ...
            fontFamily: "'Baloo Bhaijaan', cursive",
            color: "white",
            backgroundColor: hideNotInProgress ? colors.darkGreen : "#555",
          }}>
          {hideNotInProgress ? "LIVE only" : "Hide not started"}
        </button>
        {/* <Carousel autoPlay={false} animation="slide" timeout={500}> */}
        {visibleBets
          .slice()
          .sort((x, y) => {
            // Straight-bet group ticket always sorts to the bottom.
            const xStraight = x.some((b) => b.ourBet?.isStraight) ? 1 : 0;
            const yStraight = y.some((b) => b.ourBet?.isStraight) ? 1 : 0;
            if (xStraight !== yStraight) return xStraight - yStraight;
            return y[0].winAmount - x[0].winAmount;
          })
          .map((x, idx) => (
            <>
              <div
                ...
                <div
                  ...
                  <p
                    ...
                    style={{
                      ...
                      textAlign: "center",
                      cursor: "pointer",
                      color: hideNotInProgress ? colors.lightGreen : "white",
                      fontFamily: "'Baloo Bhaijaan', cursive",
                    }}>
                    $
              ...
              <div
                ...
                {x?.length > 0 &&
                  x.map((bet, idx) => {
                    ...
                    // Label the period boxes to match the actual number of linescore columns
                    // (soccer extra time / penalties add columns beyond the 2 regulation halves).
                    const periodLabels = Array.from(
                      {
                        length: Math.max(
                          periods.length,
                          homeTeamLineScore?.length ?? 0,
                          awayTeamLineScore?.length ?? 0,
                        ),
                      },
                      (v, k) => k + 1,
                    );
                    ...
                    // Soccer (and other scheduled games) put "Scheduled" in shortDetail with no time,
                    // so fall back to the event's ISO datetime converted to Central time.
                    const isoTime = bet.score?.date
                      ? moment.parseZone(bet.score.date).utcOffset(-5).format("h:mm a")
                      : null;
                    const momentTime =
                      time ? moment(time, "h:mm a").subtract(1, "hour").format("h:mm a")
                      : isoTime ? isoTime
                      : "TBD";
                              ...
                              {bet.score.status.type.name !== "STATUS_SCHEDULED" &&
                                ...
                                bet.score.status.type.name !== "STATUS_POSTPONED" && (
                                  <React.Fragment>
                                    ...
                                    {bet.score?.competitions[0].format.regulation.periods === 9 && getBaseImgSrc(bet.score)}
                                  </React.Fragment>
                                )}
                            </div>
                          </div>
    ...
    return (
      <>
        ...
        <button
          ...
          {hideNotInProgress ? "Showing LIVE only" : "Hide not started"}
        </button>
        <AutoCarousel interval={6000}>
          {visibleBets
          .slice()
          .sort((x, y) => {
            // Straight-bet group ticket always sorts to the bottom.
