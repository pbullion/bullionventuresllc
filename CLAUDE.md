...
| `/quick-bets` | `/kalshi/quick-bets/candidates` + `/kalshi/quick-bets/combo` (NCAAF, NFL, MLB — the league list comes from the backend's `QUICK_BET_LEAGUES`; kalshi-live builds from 2026-09-13 use the same two; builds installed before that use `/ncaaf` + `/ncaaf-combo`, which the backend keeps and serves pregame-only). **Best N** (added 2026-09-14, `BestTicket.jsx` + `Record.jsx`): `POST /kalshi/quick-bets/best-ticket`, `POST /kalshi/quick-bets/combo` with `{ticket_id, stake_dollars}` (a separate branch — the `{legs}` body above is unchanged), `GET /kalshi/quick-bets/record`, `GET /kalshi/quick-bets/lessons`. A backend without them answers 404 and the page shows a quiet "not deployed yet" note instead. Since 2026-09-16 Best N buys at one of five fixed stakes ($5–$25, a button each, at most once per ticket), not the page's stake box, and the favorites list opens with every game checked, LIVE ones included |
